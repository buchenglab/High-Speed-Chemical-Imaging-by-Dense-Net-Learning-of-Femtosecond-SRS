...
def tiramisu_0203(para,input_size=(256,256,1),pretrained_weights = None):
...
#    up_layer_dim = (np.asarray([400,400,592,208,112,64])*0.5).astype(int)
...
#    dropout_rate = para['dropout_rate']
...
#    conv_test = Conv2D(filter_dim,1,padding = 'same',dilation_rate=3,name='test',kernel_regularizer = kerel_regularizer_)(conv_1)
    ## downsampling
    module = []
...
#    conc_0 = concatenate([bottle_layer,module[5]])
    ...
    model = Model(inputs = inputs, outputs = output_layer, name = 'tiramisu')
